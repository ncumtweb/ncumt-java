$(document).ready(function () {
    const { events, editUrlTemplate } = calendarConfig;
    ...
    $('#calendar').fullCalendar({
        ...
        eventClick: function (event) {
            ...
            document.getElementById("editEvent").addEventListener("click", function () {
                const url = editUrlTemplate.replace(':id', event.id);
                location.href = url;
            });
        },
        viewRender: function () {
                // 每次切換 view，先移除舊 icon，再加一次
                $(".fc-prev-button i").remove();
                $(".fc-next-button i").remove();
